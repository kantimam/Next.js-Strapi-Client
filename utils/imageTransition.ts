...
export default class Transition {
  ...
  private displacementImage?: HTMLImageElement;
  ...
  private resizeContainer: boolean = false;
  ...
  private frag: string = `
          ...
          void main() {
              ...
              float uvFromDistX=v_texCoord.x+u_progress*displacement;
              float uvToDistX=v_texCoord.x-(1.0-u_progress)*displacement;
              

              float fx1=fract(uvFromDistX);
              float fx2=fract(uvToDistX);
              ...
              vec2 uvFromDist=vec2(uvFromDistX, v_texCoord.y);
              vec2 uvToDist=vec2(uvToDistX, v_texCoord.y);
              ...
              //gl_FragColor = mix(colorFrom, colorTo, uvFromDist.y>0.5? 1.0 : 0.0);
          }
  ...
  constructor(
    ...
    if (options.resizeContainer) {
      this.resizeContainer = options.resizeContainer;
    }
  ...
  private loadImages(
    imageArray: string[] | HTMLImageElement[]
    ...
    return new Promise((resolve, reject) => {
      ...
      imageArray.forEach(
        (element: string | HTMLImageElement, index: number) => {
  ...
  private setCanvasDim() {
    if (this.resizeContainer && this.images) {
      // set the containers dimensions to match the first provided image
      const firstImage = this.images[0];
      const { width, height } = firstImage;
      const aspectRatio = height / width;
      this.container.style.height = `${this.container.clientWidth * aspectRatio}px`
    }
  ...
  private async create(imagesSrc: string[], displacementImageSrc: string) {
    try {
      ...
      this.createRenderer(this.images, this.displacementImage);
    } catch (error) {
      console.dir(error);
    }
  }
  ...
  private createRenderer(
    ...
    let positionLocation = this.gl.getAttribLocation(
      this.shaderProgram,
      "a_position"
    );
    ...
    this.uProgressLocation = this.gl.getUniformLocation(
      this.shaderProgram,
      "u_progress"
    );
    ...
    const uImage0Location = this.gl.getUniformLocation(
      this.shaderProgram,
      "u_image0"
    );
    const uImage1Location = this.gl.getUniformLocation(
      this.shaderProgram,
      "u_image1"
    );
    // lookup the sampler location of the displacement texture
    const uImageDisplaceLocation = this.gl.getUniformLocation(
      this.shaderProgram,
      "u_imageDisplace"
    );

    // set resolution
    this.gl.uniform2fv(this.uResolutionLocation, [
